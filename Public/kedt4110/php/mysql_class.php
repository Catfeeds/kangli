...
class cls_mysql
{
    ...
    {
        if (defined('WWW_CHARSET'))
        {
            $charset = strtolower(str_replace('-', '', WWW_CHARSET));
        }

        if (defined('WWW_ROOT') && !$this->root_path)
        {
            $this->root_path = WWW_ROOT;
        }
    ...
    function query($sql, $type = '')
    {
        ...
        //保存查询日志 常量DEBUG_MODE=8时，则记录查询日志
        if (defined('DEBUG_MODE') && (DEBUG_MODE & 8) == 8)
        {
            $logfilename = $this->root_path . 'data/querylog/mysql_query_' . $this->dbhash . '_' . date('Y_m_d') . '.log';
            $str = $sql . "\n\n";
