...
<html>
	...
	<head>
		...
		<title>
			<?php echo (C("QY_COMPANY")); ?>-确认发货</title>
	...
	<script type="text/javascript">
    	...
    	$("#finishship_sumbit").click(function(){
            ...
            try {
				$.ajax({
					type: 'POST',
					url: '<?php echo U('./Kangli/Orders/odfinishship');?>',
					...
					success: function (data) {
						...
						if (stat == 1) {
							 // alert(data.msg);
							 $.toast(data.msg);
							 // location.href = "<?php echo U('./Kangli/Orders/dlorderdetail?od_id='.$od_id.'&odbl_id='.$odbl_id.'&od_state='.$od_state.'');?>";
							 location.href = "<?php echo U('./Kangli/Orders/dlorders?od_id='.$od_id.'&odbl_id='.$odbl_id.'&od_state='.$od_state.'');?>";
							 return false; 
						}else{
							$.toast(data.msg);
